...
def adrply(r,id):
        ...
        if r.method=="POST":
            ...
            name=r.POST.get('name')
            email=r.POST.get('email')
            replay=rply.objects.create(name=name,email=email,message=message)
            replay.save()
            try:
                send_mail('Replay to user', f'{message}','settings.EMAIL_HOST_USER', [email],fail_silently=False)
            except Exception as E:
                print("Error",E)
            # return render(r,'adrply.html',{'alert':alert})
        return render(r,'adrply.html',{'obj':obj})


def inbox(r):
        re=cont.objects.get(pk=id)
        return render(r,'inbox.html',{'st':re})
...
def adfmview(r):
        ...
        return render(r,'adfmview.html',{'st':re})
...
def addtocart(r,id):
    if 'user' in r.session:
        ...
        if cart.objects.filter(cartitm=itm,user_details=user_details).exists():
            ...
            citm.tprice=citm.tprice+itm.price
            citm.count=citm.count+1
            # print("CITM B4",citm.cartitm.crt,itm.crt)
            # citm.cartitm.crt=1
            # print("CITM",citm.cartitm.crt,itm.crt)
            itm.save()
            citm.save()
            return redirect(usrdisplay)
            # print(citm)
...
#         return render(r,'myacc.html',{'uu':uu})
#     else:
...
def mycart(r):
    if 'user' in r.session:
        length=chckcart(r.session['user']['name'])
        if length==0:
            messages.info(r,'The cart is empty...')
...
def Usrdisplayy2(r,id):   
        ...
        return render(r,'Usrdisplayy2.html',{'i':re,'citm':citm,'itms':itms,'length':length})
...
def order(r):
    ...
    return render(r,'order.html',{'pay':data,'ca':data1})
